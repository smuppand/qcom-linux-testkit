...
TESTNAME="AI_ML_Tflite_Benchmark_CPU_3_thread_inception_v3_quant.tflite"
RES_FILE="$SCRIPT_DIR/$TESTNAME.res"

# usage
# Prints the fixed CPU benchmark CLI contract and returns without target changes.
usage() {
...
tflite_benchmark_defaults cpu || exit 2
parse_args "$@" || {
    usage >&2
    exit 2
}
