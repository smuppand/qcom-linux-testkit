...
if [ "$need_vp9_clip" -eq 1 ] && [ "$testMode" != "encode" ]; then
  ...
  if [ -f "$vp9_clip_webm" ]; then
    ...
    # Prefer local path (dir or tar*) if user provided it
    if [ -n "$clipPath" ] && command -v prepare_vp9_from_local_path >/dev/null 2>&1; then
      log_info "Attempting VP9 clip prep from local path: $clipPath"
      if prepare_vp9_from_local_path "$clipPath" "$OUTDIR" "$vp9_clip_ivf" "$vp9_clip_webm"; then
        log_info "VP9 clip prepared from local path"
      else
        log_warn "Local VP9 clip prep failed: $clipPath"
      fi
    fi

    # If still not present, try URL download
    if [ ! -f "$vp9_clip_webm" ] && [ ! -f "$vp9_clip_ivf" ]; then
      log_info "VP9 clip not found locally; attempting download from URL..."
      if extract_tar_from_url "$clipUrl" "$OUTDIR"; then
        log_pass "VP9 clips downloaded and extracted successfully"
      else
        log_warn "VP9 clip download/extract failed (offline or URL issue)"
      fi
    fi

    if [ ! -f "$vp9_clip_ivf" ] && [ ! -f "$vp9_clip_webm" ]; then
    ...
    if [ ! -f "$vp9_clip_ivf" ] && [ ! -f "$vp9_clip_webm" ]; then
      log_warn "VP9 clip not prepared (decode will SKIP if requested)"
    else
      # If we already have WebM, we are done
      if [ -f "$vp9_clip_webm" ]; then
        log_info "VP9 WebM clip ready: $vp9_clip_webm"
      else
        # Convert IVF -> WebM
        mux=""
