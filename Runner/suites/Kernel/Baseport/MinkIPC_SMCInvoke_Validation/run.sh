...
TESTNAME="MinkIPC_SMCInvoke_Validation"
RES_FILE="$SCRIPT_DIR/$TESTNAME.res"
CLIENT_PATH="${SMCINVOKE_CLIENT:-}"
ITERATIONS=1

usage() {
    echo "Usage: $0 [--client PATH] [--iterations N]"
}
...
if [ -z "$CLIENT_PATH" ]; then
    for candidate in /usr/bin/smcinvoke_client /usr/sbin/smcinvoke_client \
        /usr/local/bin/smcinvoke_client /usr/lib/minkipc/smcinvoke_client \
        /usr/libexec/minkipc/smcinvoke_client; do
        if [ -x "$candidate" ]; then
...
RUN_LOG="$SCRIPT_DIR/smcinvoke_client.log"
rm -f "$RUN_LOG" "$RES_FILE"
log_info "Starting $TESTNAME on OS=$OS_ID"
log_info "Running public MinkIPC diagnostic: $CLIENT_PATH -d $ITERATIONS"

"$CLIENT_PATH" -d "$ITERATIONS" > "$RUN_LOG" 2>&1
CLIENT_RC=$?
head -n 80 "$RUN_LOG" 2>/dev/null | while IFS= read -r line; do
    if [ -n "$line" ]; then
...
if [ "$CLIENT_RC" -eq 0 ]; then
    log_fail "$TESTNAME FAIL: smcinvoke_client did not report TEST PASSED (see $RUN_LOG)"
    echo "$TESTNAME FAIL" > "$RES_FILE"
    exit 1
fi

log_fail "$TESTNAME FAIL: smcinvoke_client returned $CLIENT_RC (see $RUN_LOG)"
echo "$TESTNAME FAIL" > "$RES_FILE"
exit 1
