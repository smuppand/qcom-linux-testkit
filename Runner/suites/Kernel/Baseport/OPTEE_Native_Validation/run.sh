...
log_info "Configuration: xtest=$OPTEE_XTEST_BIN cases=$OPTEE_XTEST_CASES timeout=${OPTEE_XTEST_TIMEOUT}s"
log_info "[OPTEE-POLICY] device=dynamic xtest=$OPTEE_XTEST_BIN cases=$OPTEE_XTEST_CASES allowlist=$OPTEE_SAFE_XTEST_CASES"
...
if [ -z "$OPTEE_DEVICE" ]; then
    test_result_record "SKIP" "No public native OP-TEE character device was identified from impl_id or runtime driver evidence, tee_devices=$OPTEE_TEE_DEVICE_COUNT public_devices=$OPTEE_PUBLIC_DEVICE_COUNT native_candidates=$OPTEE_NATIVE_DEVICE_COUNT artifact=$DEVICE_INVENTORY"
    test_result_finish
fi
