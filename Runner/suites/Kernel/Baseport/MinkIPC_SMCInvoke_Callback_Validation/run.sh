...
TESTNAME="MinkIPC_SMCInvoke_Callback_Validation"
...
RUN_LOG="$SCRIPT_DIR/smcinvoke_callback.log"
rm -f "$RUN_LOG" "$RES_FILE"

log_info "Starting $TESTNAME on OS=$OS_ID"
log_info "Using callback test TA: $TA_DIR/tzecotestapp.mbn"
log_info "Running callback object validation: $CLIENT_PATH -c $TA_DIR $ITERATIONS"

"$CLIENT_PATH" -c "$TA_DIR" "$ITERATIONS" > "$RUN_LOG" 2>&1
CLIENT_RC=$?
...
if [ "$CLIENT_RC" -eq 0 ]; then
    log_fail "$TESTNAME FAIL: callback test did not report TEST PASSED (see $RUN_LOG)"
    echo "$TESTNAME FAIL" > "$RES_FILE"
    exit 1
fi

log_fail "$TESTNAME FAIL: callback test returned $CLIENT_RC (see $RUN_LOG)"
echo "$TESTNAME FAIL" > "$RES_FILE"
exit 1
