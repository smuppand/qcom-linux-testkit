...
# Move into testcase directory (so .res lands in the right place)
cd "$(find_test_case_by_name "$TESTNAME")" || exit 1
...
if [ -n "$MATCHES" ]; then
    ...
    # Print a few representative lines to stdout/stderr for CI log visibility
    printf '%s\n' "$MATCHES" | head -n 10 | while IFS= read -r line; do
        [ -n "$line" ] || continue
        log_error "CI-HINT: $line"
    done

    printf '%s\n' "$TESTNAME FAIL" >"$RESULT_FILE"
    exit 1
fi

log_pass "$TESTNAME : No probe/firmware/bind errors found in kernel log snapshot"
printf '%s\n' "$TESTNAME PASS" >"$RESULT_FILE"
rm -f "$LOG_FILE"
exit 0
