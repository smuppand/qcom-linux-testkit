...
Wrapper-specific (CLI flags and/or ENV equivalents):
  ...
  --no-dmesg (DMESG_SCAN=0) Disable dmesg scanning
...
JSON_OUT="${JSON_OUT:-}"; DMESG_SCAN="${DMESG_SCAN:-1}"
...
###############################################################################
# dmesg patterns (toggle with DMESG_SCAN)
###############################################################################
...
# Keep wrap_affinity minimal: only taskset if present; no numactl path.
wrap_affinity() {
    c="$1"
...
while [ "$RUN" -le "$LOOPS" ]; do
    ...
    if command -v taskset >/dev/null 2>&1; then
        ...
        if cpuset_supported; then
            ...
            log_info "CPU pinning method: none (no taskset/numactl/cpuset)"
    ...
    if [ "$DMESG_SCAN" -eq 1 ]; then
        if [ "$STRICT" -eq 1 ]; then
            if scan_dmesg_errors "$SCRIPT_DIR" "$DMESG_MODULES_STRICT" "$DMESG_EXCLUDE_STRICT"; then
                ...
                log_pass "Strict mode: no critical kernel issues detected"
            fi
        else
        ...
        log_info "dmesg scanning disabled"
    fi
...
###############################################################################
FINAL_FAIL=0
[ "$FAIL_COUNT" -gt 0 ] && FINAL_FAIL=1
[ "$STRICT" -eq 1 ] && [ "$STRICT_FAIL" -ne 0 ] && FINAL_FAIL=1
